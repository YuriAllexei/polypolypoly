use anyhow::Result;
use chrono::Utc;
use market_db::{DbEvent, DbMarket, MarketDatabase};
use reqwest::Client;
use std::sync::Arc;
...
fn market_to_db_market(market: &Market) -> Result<DbMarket> {
    ...
    Ok(DbMarket {
        ...
        start_date: market.start_date.clone().unwrap_or_default(),
        end_date: market.end_date.clone().unwrap_or_default(),
        resolution_time: market.end_date.clone().unwrap_or_default(),
        ...
        outcomes: market.outcomes.clone().unwrap_or_else(|| "[]".to_string()),
        token_ids: market.clob_token_ids.clone().unwrap_or_else(|| "[]".to_string()),
...
async fn main() -> Result<()> {
    ...
    // Load configuration
    let config = bot_config::BotConfig::load("config.yaml")?;
    ...
    info!("Starting perpetual event sync (interval: {}s)", SYNC_INTERVAL_SECS);
    ...
    loop {
        ...
        for event in events {
            ...
            let exists = db.event_exists(&event.id).await?;

            if exists {
                info!("Event {} already exists, skipping", event.id);
                continue;
            }

            info!("Processing new event: {} - {}", event.id, event.title.as_deref().unwrap_or("<no title>"));
            ...
            match db.upsert_event(db_event).await {
                Ok(_) => {
                    ...
                    // Process and save associated markets
                    let market_ids: Vec<String> =
                        event.markets.iter().map(|m| m.id.clone()).collect();
                    ...
                    if !market_ids.is_empty() {
                                ...
                                Ok(db_market) => {
                                    if let Err(e) = db.upsert_market(db_market).await {
                                        warn!(
                                            ...
                                            market.id, event.id, e
                                        );
                                    } else {
                                        info!("  Saved market {} - {}", market.id, market.question.as_deref().unwrap_or("<no question>"));
                                    }
                                }
                                Err(e) => {
                                    warn!(
                        ...
                        if let Err(e) = db.link_event_markets(&event.id, &market_ids).await {
                            warn!("Failed to link markets for event {}: {}", event.id, e);
                        }
                    }
                }
                Err(e) => {
                    error!("Failed to save event {}: {}", event.id, e);
                }
            }
        }
    ...
    info!("Sleeping for {} seconds before next cycle...", SYNC_INTERVAL_SECS);
