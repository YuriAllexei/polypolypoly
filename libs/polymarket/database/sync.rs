use super::{DatabaseError, MarketDatabase, Result, models::DbMarket};
use crate::database::SyncStats;

use chrono::Utc;
use crate::client::{GammaClient, GammaMarket};
use std::sync::Arc;
...
mod tests {
    ...
    fn test_convert_gamma_to_db() {
        use crate::client::gamma::types::Outcomes;
        ...
        let gamma = GammaMarket {
            ...
            outcomes: None, // Outcomes is an enum, not a Vec<String>
            clob_token_ids: Some("[\"0x1\",\"0x2\"]".to_string()),
            ..Default::default()
        };
