use crate::infrastructure::cache::{CacheEntry, MarketCache};
...
pub enum FilterError {
    #[error("Cache error: {0}")]
    CacheError(#[from] crate::infrastructure::cache::CacheError),

    #[error("Ollama error: {0}")]
    OllamaError(#[from] crate::infrastructure::ollama::OllamaError),
}
...
impl LLMFilter {
    ...
    pub async fn filter_markets(&mut self, markets: Vec<MarketInfo>) -> Result<Vec<MarketInfo>> {
        ...
        if !uncached.is_empty() {
            ...
            let llm_results = self
                .ollama
                .filter_markets(&self.prompt, &market_data)
                .await?;
    ...
    pub fn cache_stats(&self) -> crate::infrastructure::cache::CacheStats {
        self.cache.stats()
    }
